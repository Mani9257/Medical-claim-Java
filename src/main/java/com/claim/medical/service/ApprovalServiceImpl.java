...
public class ApprovalServiceImpl implements ApprovalService {
	...
	public ApprovalResponseDTO adminApproval(Integer claimId, Integer approverId, String claimStatus)
		...
		Optional<ClaimStatus> claimRequest = claimStatusRepository.findClaimStatusByClaimId(claimId);
		...
		if (admin.isPresent()) {
			...
			if (claimRequest.isPresent()) {
				...
				if (claimRequest.get().getFirstLevelClaimStatus()
						.equalsIgnoreCase(MedicalClaimConstants.PENDING_STATUS_BY_ADMIN)
						&& claimStatus.equalsIgnoreCase(MedicalClaimConstants.APPROVED_STATUS_BY_ADMIN)) {
					claimRequest.get().setFirstLevelClaimStatus(claimStatus);
					claimRequest.get().setApproverId(approverId);
			...
			else if (claimRequest.isPresent() && claimRequest.get().getFirstLevelClaimStatus()
					.equalsIgnoreCase(MedicalClaimConstants.APPROVED_STATUS_BY_ADMIN)) {
				claimRequest.get().setSecondLevelClaimStatus(claimStatus);
				claimRequest.get().setSecondLevelClaimStatus(claimStatus);
				log.info("senior approver ={}", claimRequest.get().getSeniorApproverId());
				claimStatusRepository.save(claimRequest.get());
